...
class Ion_auth_model extends CI_Model
{
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$data = array(
		    ...
		    'created_on' => time(),
		    'active'     => '0'//($manual_activation === false ? 1 : 0)
		);
	...
	**/
    ...
    function insert_login_activity($identity = NULL){
        $q = $this->db->get_where('users',array('email'=>$identity));
        ...
        if($this->db->insert('login_activity',$ar)){
            return true;
        }
        return false;
    }
    ...
    {
        $from_email = 'milan@vakratundasystem.in'; //change this to yours
        $subject = 'Verify Your Email Address';
        $message = 'Dear User,<br /><br />Please click on the below activation link to verify your email address.<br /><br /> http://www.vakratundasystem.in/application/floobie/index.php/auth/verify/' . md5($to_email) . '<br /><br /><br />Thanks<br />VakratundaSystem Team';
        
        //configure email settings
        $config['protocol'] = 'smtp';
        $config['smtp_host'] = 'ssl://sindhu.ewebguru.net'; //smtp host name
        $config['smtp_port'] = '465'; //smtp port number
        $config['smtp_user'] = $from_email;
        $config['smtp_pass'] = 'milan123..'; //$from_email password
        ...
        //send mail
        $this->email->from($from_email, 'vakratundasystem');
